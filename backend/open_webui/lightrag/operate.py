...
async def _process_extraction_result(
    ...
    for i, record in enumerate(fixed_records, start=1):
        ...
        # Normalize bare pipe-delimiter-pipe format (|#| -> <|#|>)
        # Some LLMs omit the angle brackets from the tuple delimiter.
        bare_delimiter = f"|{delimiter_core}|"
        record = record.replace(bare_delimiter, tuple_delimiter)

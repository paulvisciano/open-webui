...
async def openai_embed(
    ...
    async with openai_async_client:
        ...
        # Handle SDK response parsing edge case: some OpenAI-compatible servers
        # (e.g. llama-server) cause the SDK to return a list of
        # CreateEmbeddingResponse objects instead of a single
        # CreateEmbeddingResponse with a .data attribute.  This happens when
        # base_url lacks the /v1 path prefix.  In the list case, each item's
        # .embedding is already decoded but wrapped in an extra list layer,
        # e.g. [[float, float, ...]] instead of [float, float, ...].
        ...
        embeddings = []
        for dp in data_items:
            emb = dp.embedding
            if isinstance(emb, list):
                if flatten_embedding and isinstance(emb, list) and len(emb) == 1 and isinstance(emb[0], list):
                    emb = emb[0]
                embeddings.append(np.array(emb, dtype=np.float32))
            else:
                embeddings.append(np.frombuffer(base64.b64decode(emb), dtype=np.float32))
